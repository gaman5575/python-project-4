    ...
    stages {
        stage('Git Checkout') {
            steps {
                ...
                git branch: 'main',
                    url: 'https://github.com/gaman5575/python-project-4.git'
            }
        }
                ...
                withCredentials([usernamePassword(credentialsId: 'git-credentials', usernameVariable: 'GIT_CREDENTIALS_USR', passwordVariable: 'GIT_CREDENTIALS_PSW'),
                    ...
                    script {
                        ...
                        sh """
                            ...
                            git clone https://${GIT_CREDENTIALS_USR}:${GIT_CREDENTIALS_PSW}@github.com/${GIT_CREDENTIALS_USR}/python-project-4.git
                            cd python-project-4
                            ...
                            git push -u https://${GIT_CREDENTIALS_USR}:${GIT_CREDENTIALS_PSW}@github.com/${GIT_CREDENTIALS_USR}/python-project-4.git "\${new_branch}"

                            echo "Building the Docker image"
                            docker build -t "${DOCKER_USERNAME}/python-project-4:\${new_version}" .

                            

                            echo "Pushing the Docker image to Docker Hub"
                            docker login -u "${DOCKER_USERNAME}" -p "${DOCKER_PASSWORD}"
                            docker push "${DOCKER_USERNAME}/python-project-4:\${new_version}"

                            cd ..
                            rm -rf python-project-4
                        """
                    }
                }
            }
